...
},
{
  ...
  blocks:[{ rows:[
    ["06:45–07:15","湖畔散步","哈修塔特湖畔","晨間短走與拍照，可停留 30 分鐘",0,"hal-marktplatz"],
    ["07:15–08:10","早餐／退房","哈爾貝格公寓","早餐、整理行李、完成退房，依住宿指示將行李送回 P1",0,"hal-stay"],
    ["08:10–08:40","步行","住宿區 → 哈修塔特鹽礦纜車山下站 Salzwelten","—",0,"salzwelten-tal"],
    ["08:40–09:00","入場準備","鹽礦纜車山下站","出示鹽礦與纜車套票、排隊準備搭首班纜車。購票前確認鹽礦與新纜車已正式重新開放",0,"salzwelten-tal"],
    ["09:00–09:05","纜車","鹽礦纜車上山","乘車 2 分鐘，配置 5 分鐘",0,"salzwelten-tal"],
    ["09:05–09:25","步行","山頂纜車站 → 鹽礦導覽入口","步行 20 分鐘",0,"salzwelten"],
    ["09:25–09:30","集合","鹽礦入口","完成報到與集合",0,"salzwelten"],
    ["09:30–11:00","鹽礦導覽","哈修塔特鹽礦 Salzwelten Hallstatt","**規劃 09:30 場次**，導覽 90 分鐘。尚未購票，不視為已預約",1,"salzwelten"],
    ["11:00–11:20","步行","鹽礦出口 → 天空步道","步行 20 分鐘",0,"skywalk"],
    ["11:20–11:50","景點","哈修塔特天空步道 Hallstatt Skywalk","山頂湖景與拍照，可停留 30 分鐘",0,"skywalk"],
    ["11:50–12:00","纜車／步行","山頂站 → 山下站","**12:00 離開山下站**",0,"salzwelten-tal"],
    ["12:00–13:00","行李／午餐","哈修塔特 P1","取回行李，P1 周邊簡單午餐或食用預備餐點，可停留 60 分鐘；**13:00 正式開車離開**",0,"hal-p1"],
    ["13:00–15:20","移動","哈修塔特 P1 → 普里恩 Prien am Chiemsee","—",0,"prien"],
    ["15:20–15:50","咖啡／湖畔","席勒咖啡館 Schiller Chiemsee、基姆湖畔","咖啡、蛋糕或蘋果捲後湖畔短走，可停留 30 分鐘；點心休息，不安排正餐",0,"prien"],
    ["15:50–16:55","移動","普里恩 → Motel One München-Hauptbahnhof","—",0,"muc-stay2"],
    ["16:55–17:35","住宿／停車","Motel One München-Hauptbahnhof","入住、卸下全部行李，依住宿確認結果停車",0,"muc-stay2"],
    ["17:35–20:00","採買／藥局","新豪瑟街 Neuhauser Straße、考芬格街、國際路德維希藥局","藥局星期一營業至 20:00；**先買藥再逛商店**",0,"neuhauser"],
...
const CHECKLIST = [
  ...
  { id:"net", title:"網路與通訊", items:[
    ...
    ["net2","確認自己的手機支援 eSIM 並先裝好","出國前就要裝好，落地才弄會卡住"],
  ...
  { id:"fx", title:"外幣與支付", items:[
    ...
    ["fx4","（駕駛人）本人名義的信用卡正卡","SIXT 押金只認駕駛人本人的卡，不接受他人卡片"],
  ]},
  { id:"doc", title:"證件與駕駛", items:[
    ["doc1","護照效期確認","需超過 2027/04"],
    ["doc2","（駕駛人）國際駕照 ＋ 台灣駕照正本","德奧租車兩者都要出示，缺一不可"],
    ["doc3","護照影本與電子檔備份",""],
    ["doc4","10/05 當晚清點自己的證件","護照、駕照、信用卡；總檔 Day 1 列為當晚全員確認事項"],
  ]},
  { id:"tkt", title:"票券", items:[
    ["tkt1","新天鵝堡 Tour 445","10/06 11:45 固定入場，成人票已購買；確認自己手機能離線開啟 QR code"],
    ["tkt6","國王湖遊船來回票","10/10 Fahrt-Nr. 10、08:45 開船，4 張成人票已購買。**08:25 前到 Seelände 報到**，錯過即失效；回程票要留著"],
...
const TICKETS = {
  ...
  later:[
    { name:"薩爾斯堡卡 24 小時", when:"10/08 晚線上購買，使用日選 10/09（Day 5）", price:"€38／人",
      how:"含要塞纜車與各館入場。四人各自收到個人卡連結，**存成離線可讀**，不要只留在信箱。" },
    { name:"薩爾斯堡住宿交通票", when:"10/08 入住時向住宿方領取（Day 4）", price:"含於房價",
      how:"每人一張，領到後自己收好。" },
    { name:"哈修塔特鹽礦＋纜車套票", when:"10/12 09:30 場次（Day 8）", price:"€49／人",
      how:"**尚未購票**。購買前先確認鹽礦與新纜車已正式重新開放。" },
    { name:"楚格峰／北鏈纜車", when:"Day 3、Day 4 當天現場", price:"依當日票價",
      how:"**一律不預購**，依 Webcam 與營運狀況現場決定。官方明示天候不佳不退改。" },
  ],
  notes:[
    ["原始票券不放網站","四份 PDF 含可掃描的 QR，放上公開網頁等於任何人都能先掃掉。本頁只列文字資訊，票檔另外私下保管。"],
    ["離線準備","出發前把 PDF 存進手機相簿或離線檔案 App，並實測在飛航模式下能開啟。同時列印一份黑白紙本放隨身包。"],
    ["兩張都不可改期","新天鵝堡與國王湖都綁定日期與時間。行程若因天候調整，這兩個時間點是固定錨點，其餘往前後挪。"],
  ],
};
